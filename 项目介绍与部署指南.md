...
1. 用户发布商品后，商品状态默认为"待审核"
2. 管理员通过后台审核商品
3. 审核通过后，商品状态变为"已审核"并在前端展示
4. 商品售出后，状态变为"已售出"
...
3. **构建项目**
   ...
   构建成功后，会在target目录下生成`campusmarket.war`文件。

4. **部署到Tomcat**

   - 将`campusmarket.war`文件复制到Tomcat的webapps目录下
   - 启动Tomcat服务器
   - Tomcat会自动解压和部署war文件
...
1. 默认管理员账号仅用于初始登录，请及时修改密码
2. 定期备份数据库以防止数据丢失
3. 生产环境中应加强安全措施，如密码加密存储
4. 建议部署HTTPS以保障传输安全
5. 定期清理过期数据和无效文件